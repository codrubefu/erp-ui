...
export default function ArticlesList() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Nu am putut incarca articles.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const remove = async () => {
    ...
    try {
      ...
      setDeleting(null);
      setToast({ type: 'success', message: 'Article a fost sters.' });
      await load();
    } catch (err) {
      setToast({ type: 'error', message: err instanceof Error ? err.message : 'Nu am putut sterge article.' });
    }
  };
  ...
  return (
    <ProtectedRoute requiredRights={['articles.view', 'articles.manage']}>
      ...
      <SectionCard title="Articles" action={canCreateArticle ? <Link to="/erp/articles/create" className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2 text-sm font-semibold text-white"><Plus className="h-4 w-4" />Adauga Article</Link> : null}>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
          <label className="block md:col-span-2">
            <span className="mb-2 block text-sm font-medium text-slate-700">Cautare titlu/descriere</span>
          ...
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-slate-700">Group</span>
            <select value={filters.group} onChange={(event) => setFilters((prev) => ({ ...prev, group: event.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-100"><option value="">Toate</option>{groups.map((group) => <option key={group.id} value={group.id}>{labelFor(group)}</option>)}</select>
          </label>
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-slate-700">Location</span>
            <select value={filters.location} onChange={(event) => setFilters((prev) => ({ ...prev, location: event.target.value }))} className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-100"><option value="">Toate</option>{locations.map((item) => <option key={item.id} value={item.id}>{labelFor(item)}</option>)}</select>
          </label>
          <div className="flex items-end gap-2 md:col-span-2">
            <button onClick={() => void load()} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white"><Search className="h-4 w-4" />Cauta</button>
            {canCreateArticle ? <Link to="/erp/articles/create" className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 py-3 text-sm font-semibold text-white"><Plus className="h-4 w-4" />Adauga Article</Link> : null}
          </div>
        </div>
        {error ? <p className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{error}</p> : null}
        <div className="mt-6 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead><tr className="border-b text-slate-500"><th className="pb-3">Titlu</th><th className="pb-3">Descriere</th><th className="pb-3">Groups</th><th className="pb-3">Locations</th><th className="pb-3 text-right">Actiuni</th></tr></thead>
            ...
            )) : <tr><td colSpan={5} className="py-10 text-center text-slate-500">{loading ? 'Se incarca articles...' : 'Nu exista articles pentru filtrele curente.'}</td></tr>}</tbody>
          </table>
        </div>
      </SectionCard>
      {deleting && hasAnyRight(['articles.delete', 'articles.manage']) ? <div className="fixed inset-0 z-40 grid place-items-center bg-slate-950/40 p-4"><div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl"><h3 className="text-lg font-semibold text-slate-900">Confirmare stergere</h3><p className="mt-2 text-sm text-slate-600">Stergi article {deleting.title}?</p><div className="mt-6 flex justify-end gap-2"><button onClick={() => setDeleting(null)} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold">Anuleaza</button><button onClick={() => void remove()} className="rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white">Sterge</button></div></div></div> : null}
      </div>
    </ProtectedRoute>
  );
}
