import { ChevronLeft, ChevronRight, Filter, MessageSquare, Phone, RefreshCw } from 'lucide-react';
        ...
        <div>
          <SectionCard title={t('sms.logTitle')}>
            <div className="space-y-3 text-sm">
              <div className="rounded-2xl bg-slate-50 p-3 text-slate-600">{t('sms.showingCount', { count: pagination.total })}</div>
              <div className="rounded-2xl bg-slate-50 p-3 text-slate-600">{t('users.pageOf', { page: pagination.current_page, lastPage: pagination.last_page })}</div>
              <div className="rounded-2xl bg-slate-50 p-3 text-slate-600">{loading ? t('sms.loadingList') : t('sms.latestStatus')}</div>
            </div>
          </SectionCard>
        </div>
      </div>
